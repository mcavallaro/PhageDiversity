...
#' run validation
#' 
valid_res_cj <- vector("list",length = length(samples_1K))
names(valid_res_cj) <- samples_1K
...
#' Chao Jost estimator
valid_res_cj[[i]] <- sapply(trainsize,function(s1){
                        intval(estim1=ChaoJost,
                               trainsize = s1,
                        M = freq_table,type = "freq_table",
                        n = valreps,out = "rawdist" 
                        )},simplify = "matrix")

rownames(valid_res_cj[[i]]) <-  c(rep("abs_err",valreps),
                               rep("NAE:",valreps))
  
                             #c(rep("true",valreps),
                             #rep("estim",valreps))
                             
colnames(valid_res_cj[[i]])[1:length(trainfrac)] <- paste0("ChaoJost:",trainfrac)

}
save(valid_res_cj,
     file = paste0("intval_cj_n",valreps,"_train5_rawdist_",yr,".RData"))
}
